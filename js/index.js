...
document.getElementById("load").addEventListener("click", function(e) {
  ...
  var TheSongName = document.getElementById("TheSongName").value;
  ...
  if(seed) {
    // if(seed.match(/^[A-Za-z0-9]+$/g)) {
	  ...
	  if(TheSongName)
	  {
		  ...
		  APP.MusicGenerator.init(TheSongName);
	  }
	  else
	  {
		var TheSongName = "Untitled Song";  
		APP.MusicGenerator.init(TheSongName);
	  }
    ...
    alert("Please enter a seed. If a seed wasn't provided, refresh the page.");
...
document.getElementById("seed").value = randString(64);	

(function() {

  APP.MusicGenerator = {};

  var _v_key      = document.getElementById("seed").value; // change this to refresh the seed
  ...
  function _setSynth() {
    ...
    _synth = new Tone.PolySynth(1, Tone.AMSynth).toMaster();
    _synth = new Tone.PolySynth(6, Tone.Synth).toMaster();
    _synth.set({
      oscillator: {
        type: "sine"
      },
